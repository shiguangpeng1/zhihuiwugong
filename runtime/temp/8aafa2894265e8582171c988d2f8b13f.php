<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:92:"F:\phpStudy\PHPTutorial\WWW\thinkphp\public/../application/admin\view\admin\waitMission.html";i:1558745013;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
  ...
  <div class="layui-side layui-bg-black left-side">
    <div class="layui-side-scroll">
      ...
      <ul class="layui-nav layui-nav-tree">
        ...
        <li class="layui-nav-item layui-nav-itemed"><a href="javascript:;">待办处理事件</a></li>
      </ul>
    </div>
  </div>
...
<script>
  layui.use(['table', 'element'], function () {
    ...
    table.render({
      elem: '#mytable',
      url: "<?php echo url('admin/waitMission'); ?>",
      toolbar: '#toolbarDemo',
      title: '用户数据表',
      method: 'post',
        ...
        if (result == 0) {
          layer.confirm('确定同意该申请吗?', function (index) {
            $.ajax({
              ...
              type: 'post',
              dataType: 'json',
              data: {status: status, result: result, time:time, handle_id: handle_id},
        ...
        if (result == 0) {
          layer.confirm('确定拒绝该申请吗?', function (index) {
            $.ajax({
              ...
              type: 'post',
              dataType: 'json',
              data: {status: status, result: result, date: date, handle_id: handle_id},
              success: function (res) {
                table.reload('testReload');
                layer.close(index);
              }
